...
# --- PATH FIX: Changed "FrontEnd" to "Front_end" to match your actual folder ---
...
def load_model():
    ...
    # --- ADDED: Explicit GPU check and logging to match your previous file ---
    ...
    model = AutoModelForCausalLM.from_pretrained(
        REPO_ID,
        torch_dtype="auto",
        device_map="auto" # This automatically pushes the model to your GPU
    )
    logger.info(f"Model ready and loaded into {device.upper()}!")
...
def generate(question: str, max_tokens: int = 512, temperature: float = 0.3) -> str:
    ...
    # model.device detects your GPU dynamically because of device_map="auto"
...
# Ensure the static files are only mounted if the directory exists
...
async def serve_ui():
    ...
    if not index_path.exists():
        return {"message": f"Frontend index.html not found at: {index_path}"}
...
async def chat(req: ChatRequest):
    ...
    return ChatResponse(
        ...
        device=str(model.device) # This will return 'cuda:0' in your API response
    )
